...
contract FactsRegistry {
    ...
    function proveAccount(address account, uint16 accountFieldsToSave, Types.BlockHeaderProof calldata headerProof, bytes calldata accountTrieProof) external {
        ...
        if (_readBitAtIndexFromRight(accountFieldsToSave, 0)) {
        ...
        if (_readBitAtIndexFromRight(accountFieldsToSave, 1)) {
        ...
        if (_readBitAtIndexFromRight(accountFieldsToSave, 2)) {
        ...
        if (_readBitAtIndexFromRight(accountFieldsToSave, 3)) {
    ...
    function _readBitAtIndexFromRight(uint16 bitmap, uint256 index) internal pure returns (bool value) {
