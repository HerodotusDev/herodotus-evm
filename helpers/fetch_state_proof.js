...
async function main() {
  ...
  if (!["account", "slot"].includes(proofType)) throw new Error("Argument proof type is invalid");
  ...
  if (!OFFLINE) {
    ...
    if (!proof) throw new Error(`Proof slot: ${slotArgStr} of ${accountArgStr} at block ${blockArgStr} is not cached`);
  }
