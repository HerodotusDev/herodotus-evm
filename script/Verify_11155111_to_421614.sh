...
echo -n "L1ToArbitrumMessagesSender: "
read L1_TO_ARBITRUM_MESSAGES_SENDER

HEADERS_STORE=$HEADERS_STORE \
    FACTS_REGISTRY=$FACTS_REGISTRY \
    SIMPLE_MESSAGES_INBOX=$SIMPLE_MESSAGES_INBOX \
    npx hardhat run script/Verify_11155111_to_421614_part1.ts --network arbitrumSepolia

SIMPLE_MESSAGES_INBOX=$SIMPLE_MESSAGES_INBOX \
    NATIVE_PARENT_HASHES_FETCHER=$NATIVE_PARENT_HASHES_FETCHER \
    L1_TO_ARBITRUM_MESSAGES_SENDER=$L1_TO_ARBITRUM_MESSAGES_SENDER \
    npx hardhat run script/Verify_11155111_to_421614_part2.ts --network sepolia
