...
const chainId = process.env.WORLD_CHAIN_SEPOLIA_CHAINID || "";
const aggregatorsFactory =
  process.env.WORLD_CHAIN_SEPOLIA_AGGREGATORS_FACTORY || "";

if (!chainId) {
  throw new Error("WORLD_CHAIN_SEPOLIA_CHAINID is not set in .env");
}
if (!aggregatorsFactory) {
  throw new Error("WORLD_CHAIN_SEPOLIA_AGGREGATORS_FACTORY is not set in .env");
}
